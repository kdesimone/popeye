...
def error_function(parameters,ts_actual,deg_x,deg_y,stim_arr):
    """
    ...
    if np.abs(x) > np.floor(np.max(deg_y))-1:
        return np.inf
...
def adaptive_brute_force_grid_search(bounds,epsilon,rounds,ts_actual,deg_x,deg_y,
                                     stim_arr):
    """    
    ...
    while passNum <= rounds:
        ...
        phat = brute(error_function,
                 args=(ts_actual, deg_x, deg_y, stim_arr),
                 ranges=bounds,
                 Ns=5,
                 finish=fmin_powell)
