...
First pass at a stimulus model for abstracting the qualities and functionality
of a stimulus into an abstract class.  For now, we'll assume the stimulus model
only pertains to visual stimuli on a visual display over time (i.e., 3D).
Hopefully this can be extended to other stimuli with an arbitrary number of
dimensions (e.g., auditory stimuli).
...
def resample_stimulus(stim_arr, scale_factor=0.05):
    """
    Resample a stimulus array by the given scale factor

    Parameters
    ----------
    stim_arr : ndarray
        3D array (x by y by t) containing a representation of the stimulus

    scale_factor : float
        A proportion by which to rescale the array, using bilinear
        interpolation where necessary
    

    """
    if scale_factor==1:
        resampled_arr = np.copy(stim_arr)
    else:
        dims = np.shape(stim_arr)

        resampled_arr = np.zeros((dims[0]*scale_factor,
                                  dims[1]*scale_factor,
                                  dims[2]))
...
# This should eventually be VisualStimulus, and there would be an abstract
# class layer above this called Stimulus that would be generic for
# n-dimentional feature spaces.
class Stimulus(object):
    ...
    def __init__(self, stim_path, viewing_distance, screen_width, scale_factor):
        ...
        self.stim_arr = load_stimulus_file(self.stim_path)
        ...
        self.ppd = (np.pi*self.pixels_across/np.arctan(
            self.screen_width/self.viewing_distance/2.0)/360.0) # degrees of
                                                                # visual angle
        
        # generate the coordinate matrices
        self.deg_x, self.deg_y = generate_coordinate_matrices(
                         self.pixels_across, self.pixels_down, 1.0)
        # generate the coarse arrays
        self.deg_x_coarse, self.deg_y_coarse = generate_coordinate_matrices(
            self.pixels_across, self.pixels_down, scale_factor)
        self.stim_arr_coarse = resample_stimulus(self.stim_arr,
                                                 self.scale_factor)
