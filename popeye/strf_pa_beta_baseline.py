...
class SpatioTemporalModel(PopulationModel):
    ...
    def generate_ballpark_prediction(self, theta, spatial_sigma, temporal_sigma, weight, beta, baseline):
    ...
    def generate_prediction(self, theta, spatial_sigma, temporal_sigma, weight, beta, baseline):
...
class SpatioTemporalFit(PopulationFit):
    ...
    @auto_attr
    def msg(self):
        txt = ("VOXEL=(%.03d,%.03d,%.03d)  TIME=%.03d  RSQ=%.02f  THETA=%.02f  SSIGMA=%.02f  TSIGMA=%.04f  WEIGHT=%.02f  BETA=%.02f  BASE=%.02f"
            %(self.voxel_index[0],
              self.voxel_index[1],
              self.voxel_index[2],
