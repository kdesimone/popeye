...
class SpatioTemporalFit(PopulationFit):
    ...
    @auto_attr
    def prediction(self):
        return compute_model_ts(self.x, self.y, self.spatial_sigma, self.temporal_sigma, self.weight, self.beta, self.baseline,
                                self.model.stimulus.deg_x,
                                self.model.stimulus.deg_y,
                                self.model.stimulus.stim_arr,
                                self.tr_length,
    ...
    @auto_attr
    def msg(self):
        txt = ("VOXEL=(%.03d,%.03d,%.03d)   TIME=%.03d   RSQ=%.02f  THETA=%.02f   RHO=%.02f   SSIGMA=%.02f   TSIGMA=%.02f   WEIGHT=%.02f   BETA=%.02f   BASE=%.02f"
            %(self.voxel_index[0],
              self.voxel_index[1],
              self.voxel_index[2],
